# manual_backfill/departures_history_backfill.py
...
from save_departures import save_departure_flights
...
SLEEP = 1
MAX_RETRY_ROUNDS = 3  # retry API call this many times
RETRY_SLEEP = 5       # wait seconds between retries
...
def backfill_departures(date):
    """Fetch and save all departures for Germany airports on a given date."""
    ...
    for airport in airports:
        ...
        print(f"Fetching DEPARTURES {iata} {date}")

        # Retry loop for API requests
        raw = None
        for round_num in range(1, MAX_RETRY_ROUNDS + 1):
            raw = get_flights_history(iata, "departure", date)
            if isinstance(raw, list):
                ...
                # API returned error message (404 or missing airport)
        ...
        for flight in raw:
            ...
            record = {
                "key": f"{dep_time}_{dep_iata}_{f_info.get('iataNumber')}",
    ...
    # Save all flights
    save_departure_flights(flights_all)
    print(f"Saved {len(flights_all)} departures to Neon PostgreSQL")
    if skipped_airports:
